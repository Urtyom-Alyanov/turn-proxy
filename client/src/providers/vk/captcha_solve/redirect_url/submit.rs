...
use crate::providers::vk::captcha_solve::redirect_url::{ChallengeAnswer, ChallengeMeta, DEBUG_INFO, reverse_proxy::solve_via_reverse_proxy, vk_api_request::method_call};

/// Отправляет на сервера ВКонтукле решение и тихо, без суеты, завершает сессию капчи, если ошибка - запускает реверсивный прокси сервер
/// Ответ - `success_token`
...
{
  let mut body: HashMap<String, String> = HashMap::from([
    ("debug".to_owned(), DEBUG_INFO.to_owned())
  ]);
  ...
  let resp =
    method_call(client, "captchaNotRobot.check", body).await?;
