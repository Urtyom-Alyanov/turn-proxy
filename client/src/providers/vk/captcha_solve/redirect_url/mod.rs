...
use crate::providers::vk::captcha_solve::redirect_url::{fetch::fetch_challenge, human_emulating::{HumanMetrics, create_captcha_environment}, image_slider_solver::solve_picture, proof_of_work::{PowChallenge, solve_pow_async}, submit::submit_captcha};
...
mod image_slider_solver; // Решает задачу поставленную пользователю путём сопоставления стыков
mod fetch; // Сама задача
mod submit; // Отправка решения

pub(super) const DEBUG_INFO: &str = "1d3e9babfd3a74f4588bf90cf5c30d3e8e89a0e2a4544da8de8bbf4d78a32f5c";
...
async fn human_answer(client: &Client, slider_settings: Option<String>, meta: ChallengeMeta) -> Result<String> {
...
/// Решает все задачи
async fn get_captcha_answer(client: &Client, challenge: Challenge) -> Result<ChallengeAnswer>
{
  ...
  Ok(ChallengeAnswer { answer: answer?, hash: hash?? })
}
