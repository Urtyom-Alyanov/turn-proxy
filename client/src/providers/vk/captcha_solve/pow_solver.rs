...
/// Решает PoW задачу, предоставленную ВК, и получает `success_token`, который можно использовать для обхода капчи.
pub async fn solve_pow_challenge(client: &Client, redirect_url: &str, session_token: Option<&str>, access_token: Option<&str>) -> Result<String>
{
  let session_token_from_url = extract_session_token(redirect_url).unwrap_or("".to_owned());

  let session_token = session_token.unwrap_or(session_token_from_url.as_str());

  let (pow_input, difficulty) = fetch_pow_challenge(client, redirect_url).await?;
...
/// Решает PoW задачу, перебирая nonce до тех пор, пока не будет найден хэш, начинающийся с нужного количества нулей
fn solve_pow(pow_input: &str, difficulty: usize) -> Result<String>
{
  ...
  //   }
  // }
  (0..10_000_000).into_par_iter().find_map_any(|nonce| {
    let data = format!("{}{}", pow_input, nonce);
    let mut hasher = Sha256::new();
    hasher.update(data.as_bytes());
    let hex_hash = hex::encode(hasher.finalize());
...
async fn submit_pow_solution(client: &Client, hash: &str, base_body: HashMap<String, String>, browser_fp: &str) -> Result<String>
{
  ...
  let connection_downlink = "[9.5,9.5,9.5,9.5,9.5,9.5,9.5,9.5,9.5,9.5,9.5,9.5,9.5,9.5,9.5,9.5]";
  ...
  let mut body = HashMap::from([
    ...
    ("connectionDownlink".to_owned(), connection_downlink.to_owned()),

    ("debug_info".to_owned(), "d44f534ce8deb56ba20be52e05c433309b49ee4d2a70602deeb17a1954257785".to_owned()),
  ]);

  body.extend(base_body);
  
  let resp = vk_api_request_internal(client, "captchaNotRobot.check", body).await?;

  if resp["status"].as_str() != Some("OK") {
    return Err(anyhow!("PoW solution rejected: {:#?}", resp));
  }

  let success_token = resp["success_token"].as_str()
...
/// Cоздаёт окружение для решения PoW задачи, вызывая необходимые методы VK API, чтобы казаться "живым" пользователем
/// 
/// Возвращает `HashMap` с `session_token` и `access_token`, которые ассоциируются с решённой PoW задачей
...
) -> Result<HashMap<String, String>>
{
  let base_body = HashMap::from([
    ...
    ("access_token".to_owned(), access_token.unwrap_or("").to_owned()),
  ]);

  vk_api_request_internal(client, "captchaNotRobot.settings", base_body.clone()).await?;
  ...
  vk_api_request_internal(client, "captchaNotRobot.componentDone", done_data).await?;
  tokio::time::sleep(Duration::from_millis(200)).await;
...
) -> Result<Value>
{
  ...
  let response = client.post(method_url)
