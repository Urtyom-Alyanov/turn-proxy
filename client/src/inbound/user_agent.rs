...
fn random_version(rng: &mut impl Rng, major_min: u32, major_max: u32)
-> String
{
  format!("{}.0.0.0", rng.random_range(major_min..=major_max),)
}
  ...
  let value = match browser {
    ...
    _ => {
      ...
      match browser {
        ...
        Browser::Opera => {
          format!("{} OPR/{}", base, random_version(&mut rng, 110, 112))
        }
        Browser::YandexBrowser => format!(
          "{} YaBrowser/{} Yowser/2.5",
          base,
          random_version(&mut rng, 23, 24)
        ),
        _ => base,
      }
    }
